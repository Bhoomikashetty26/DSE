...
def squared_distance(prediction,target):
    return np.sum((np.array(prediction)-np.array(targets))**2)
...
def neuron_output(weights,input):
    return signoid(np.dot(weights,inputs))
def binaryencode(x:int)->List(int):
    ...
    for i in range(10):
        ...
        x=x//2
    return binary
def sqerror_gradient(network,x,y):
    hidden_outputs=feed_forward(network[:1],x)[0]
    output_predictions=feed_forward(network,x)[-1]
    output_gradients=[2*output_prediction[i]-y[i])*output_predictions[i]*(1-output_predictions[i]) for i i range(len(y))]
    hidden_gradients=[hidden_output[j]*(1-hidden_outputs[j])*np.dot([network[1][i][j]*output_gradients[i] for i in range(len(output_gradienrts))],output_gradients)for j in range(len(hidden_outputs))]
